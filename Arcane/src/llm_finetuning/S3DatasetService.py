...
from src.llm_finetuning._utils.pack_dataset import pack_dataset
...
class S3DatasetService:
    ...
    def __init__(self, hf_model_id, cleaned_responses_path, is_yi_model=False):
        ...
        self.message_template = [{"role": "system", "content": SYSTEM_PROMPT},
                                 {"role": "user", "content": ''}]
        ...
        self.content_tokens = min(self.total_tokens - self.system_tokens - self.output_tokens - self.buffer_tokens, 1536)
    ...
    def _format_text_response_as_prompt(self, train_row):
        truncated_content = truncate_text_to_token_limit(text=train_row['full_content'], encoder=self.tokenizer, token_limit=self.content_tokens)
        ...
        prompt = context_prompt + json.dumps(train_row['cleaned_attributes'])
        prompt = re.sub(r'\n+', '\n', prompt)
        return prompt

    def _custom_yi_format_text_response_as_prompt(self, train_row):
        truncated_content = truncate_text_to_token_limit(text=train_row['full_content'], encoder=self.tokenizer, token_limit=self.content_tokens)
        prompt = f'USER:{SYSTEM_PROMPT}|article_start|{truncated_content}|article_end| ASSISTANT:{json.dumps(train_row["cleaned_attributes"])}{self.tokenizer.eos_token}'
        prompt = re.sub(r'\n+', '\n', prompt)
        return prompt
    ...
    def _create_dataset(self):
        ...
        # chunk dataset
        self.lm_dataset = pack_dataset(dataset, chunk_length=4096)  # We use 4096 as the maximum length for packing
    ...
    def _generate_finetune_config(self):
        ...
        self.finetune_dataset_config = {'finetune_id': f'{self.model_id}-{self.finetune_id}',
